"use client";
import { ObjectId } from "mongodb";
import { useState, useEffect } from "react";
import CheckCookie from "./CheckCookie";
import { redirect } from "next/navigation";
import { WishlistWithBook } from "@/db/models/WishlistModel";
const BASE_URL =
  process.env.NEXT_PUBLIC_BASE_URL ||
  process.env.BASE_URL ||
  "http://localhost:3000";

interface Book {
  _id: ObjectId;
  slug: string;
  image: string;
  title: string;
  author: string;
  final_price: number;
}

interface BookCardProps {
  book: Book;
}

export default function WishlistButton({ book }: BookCardProps) {
  const [isInWishlist, setIsInWishlist] = useState(false);
  const [access_token, setAccessToken] = useState("");

  const checkCookies = async () => {
    const access_token = await CheckCookie();
    setAccessToken(access_token?.value || "");
  };

  useEffect(() => {
    checkCookies();
  }, []);

  useEffect(() => {
    const checkWishlist = async () => {
      if (access_token) {
        try {
          const resp = await fetch(`${BASE_URL}/api/wishlist`, {
            headers: {
              Cookie: `access_token=${access_token}`,
            },
          });
          const data: WishlistWithBook[] = await resp.json();
          const books = data.map((book) => book.book.slug);
          setIsInWishlist(books.includes(book.slug));
        } catch (error) {
          console.error("Error checking wishlist:", error);
        }
      }
    };
    checkWishlist();
  }, [access_token, book.slug]);

  const handleAdd = async (e: React.MouseEvent) => {
    e.preventDefault();
    if (!access_token) {
      redirect("/login");
      return;
    }
    try {
      const response = await fetch(`${BASE_URL}/api/wishlist`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Cookie: `access_token=${access_token}`,
        },
        body: JSON.stringify({ bookId: book._id }),
      });
      if (response.ok) {
        setIsInWishlist(true);
      }
    } catch (error) {
      console.error("Error adding to wishlist:", error);
    }
  };

  const handleDelete = async (e: React.MouseEvent) => {
    e.preventDefault();
    try {
      const response = await fetch(`${BASE_URL}/api/wishlist/`, {
        method: "DELETE",
        headers: {
          Cookie: `access_token=${access_token}`,
        },
        body: JSON.stringify({ bookId: book._id }),
      });
      if (response.ok) {
        setIsInWishlist(false);
      }
    } catch (error) {
      console.error("Error removing from wishlist:", error);
    }
  };
  return (
    <>
      <button
        onClick={isInWishlist ? handleDelete : handleAdd}
        className="absolute top-4 right-4 z-10">
        {isInWishlist ? (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 24 24"
            className="text-red-500 w-6 h-6">
            <path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z" />
          </svg>
        ) : (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            stroke="currentColor"
            strokeWidth={1.5}
            viewBox="0 0 24 24"
            className="text-gray-400 w-6 h-6">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"
            />
          </svg>
        )}
      </button>
    </>
  );
}
